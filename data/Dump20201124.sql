...
--
-- Temporary view structure for view `avgratingdriver`
--

DROP TABLE IF EXISTS `avgratingdriver`;
/*!50001 DROP VIEW IF EXISTS `avgratingdriver`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `avgratingdriver` AS SELECT 
 1 AS `AVG_Rating`,
 1 AS `driver_id`*/;
...
INSERT INTO `questions` VALUES (1,'How friendly was the driver?','Driver'),(2,'Was the driver on time?','Driver'),(3,'Was the order damaged?','Driver'),(4,'Did the driver follow delivery instructions?','Driver'),(5,'Would you recommend this driver to a friend?','Driver'),(6,'Was the order accurate?','Restaurant'),(7,'Was the food delicious?','Restaurant'),(8,'Did the food meet your expections?','Restaurant'),(9,'Was the food warm up arrival?','Restaurant'),(10,'Would you recommend this restaurant to a friend?','Restaurant');
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `AverageRating` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `AverageRating`()
BEGIN
Select AVG(r.score) as 'AVG_Rating', c.driver_id
From ratings r 
Inner Join (Select o.order_id, o.driver_id
From campus_eats_fall2020.order o
Inner Join driver d on d.driver_id = o.driver_id) c on c.order_id = r.order_id
...
--
-- Final view structure for view `avgratingdriver`
--
...
/*!50001 VIEW `avgratingdriver` AS select avg(`r`.`score`) AS `AVG_Rating`,`c`.`driver_id` AS `driver_id` from (`ratings` `r` join (select `o`.`order_id` AS `order_id`,`o`.`driver_id` AS `driver_id` from (`order` `o` join `driver` `d` on((`d`.`driver_id` = `o`.`driver_id`)))) `c` on((`c`.`order_id` = `r`.`order_id`))) group by `c`.`driver_id` */;
...
-- Dump completed on 2020-11-24 13:11:01
